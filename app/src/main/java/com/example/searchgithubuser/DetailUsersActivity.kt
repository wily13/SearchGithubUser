...
class DetailUsersActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (dataId > 0) {
            ...
            // set detail user
            mainViewModel.setDetailGithubUser(
                dataUser.trim(),
                this@DetailUsersActivity,
                sectionPagerAdapter,
                view_pager,
                tabs,
                progressBar
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId){
            R.id.setting -> {
                val intent = Intent(this@DetailUsersActivity, SettingActivity::class.java)
